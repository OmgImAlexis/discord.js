...
class ShardClientUtil {
  ...
  constructor(client, mode) {
    ...
    if (mode === 'process') {
      ...
      client.on('ready', () => {
        (<any>process).send({ _ready: true });
      });
      client.on('disconnect', () => {
        (<any>process).send({ _disconnect: true });
      });
      client.on('reconnecting', () => {
        (<any>process).send({ _reconnecting: true });
      });
    } else if (mode === 'worker') {
  ...
  static shardIDForGuildID(guildID, shardCount) {
    ...
    if (shard < 0) throw new DiscordError('SHARDING_SHARD_MISCALCULATION', shard, guildID, shardCount);
    return shard;
  }
}
