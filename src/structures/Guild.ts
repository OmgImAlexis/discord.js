...
class Guild extends Base {
 ...
 FIXME @param {Client} client The instantiating client
   ...
   * @returns {?string}
   */
  iconURL({ format, size, dynamic }: FIXME = {}) {
   ...
   * @returns {?string}
   */
  discoverySplashURL({ format, size }: FIXME = {}) {
   ...
   */
  equals(guild: FIXME) {
    // @TODO: Remove the need for the following ts-expect-error commands
    let equal =
      guild &&
