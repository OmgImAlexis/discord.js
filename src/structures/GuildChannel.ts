...
class GuildChannel extends Channel {
  ...
  rawPosition: FIXME;
  parentID: FIXME;
  permissionOverwrites: FIXME;
  bitrate: FIXME;
  userLimit: FIXME;
  topic: FIXME;
  nsfw: FIXME;
  rateLimitPerUser: FIXME;
  ...
  overwritesFor(member, verified = false, roles: FIXME = null) {
    if (!verified) member = this.guild.members.resolve(member);
    if (!member) return [];

    roles = roles || member.roles.cache;
    const roleOverwrites: FIXME = [];
  ...
  memberPermissions(member) {
    ...
    const overwrites: FIXME = this.overwritesFor(member, true, roles);
   ...
   *   .catch(console.error);
   */
  setParent(channel, { lockPermissions = true, reason }: FIXME = {}) {
    return this.edit(
      {
   ...
   *   .catch(console.error);
   */
  setPosition(position, { relative, reason }: FIXME = {}) {
   ...
   *   .catch(console.error);
   */
  createInvite({ temporary = false, maxAge = 86400, maxUses = 0, unique, reason }: FIXME = {}) {
    return this.client.api
      .channels(this.id)
      .invites.post({
        data: {
