...
import StreamDispatcher from '../dispatcher/StreamDispatcher';
...
class BasePlayer extends EventEmitter {
  ...
  playPCMStream(stream, options, streams: FIXME = {}) {
    this.destroyDispatcher();
  ...
  playOpusStream(stream, options, streams: FIXME = {}) {
    this.destroyDispatcher();
    streams.opus = stream;
