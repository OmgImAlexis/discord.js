...
import { browser, Status, Events, ShardEvents, OPCodes, WSEvents } from '../../util/Constants';
...
const zlib = browser ? undefined : tryLoad('zlib-sync');
...
class WebSocketShard extends EventEmitter {
  ...
  sessionID: FIXME;
  ping: FIXME;
  lastPingTimestamp: FIXME;
  lastHeartbeatAcked: FIXME;
  connection: FIXME;
  inflate: FIXME;
  connectedAt: FIXME;
  expectedGuilds: FIXME;
  readyTimeout: FIXME;
  helloTimeout: FIXME;
  heartbeatInterval: FIXME;
  ratelimit: FIXME;
  ...
  connect() {
    ...
    State: ${CONNECTION_STATE[this.connection.readyState]}`);
      ...
      const wsQuery: FIXME = { v: client.options.ws.version };
   ...
   * @private
   */
  destroy({ closeCode = 1000, reset = false, emit = true, log = true }: FIXME = {}) {
    if (log) {
      this.debug(`[DESTROY]
